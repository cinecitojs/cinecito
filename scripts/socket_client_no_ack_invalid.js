...
client.on('connect', () => {
  ...
  setTimeout(() => {
    console.log('sending invalid message via socket (no content)');
    client.emit('send-message', { roomId });
  }, 500);
});
client.on('message-error', (e) => { console.log('message-error', JSON.stringify(e)); client.close(); process.exit(0); });
client.on('message', (msg) => { console.log('received message', JSON.stringify(msg)); client.close(); process.exit(0); });
